 ...
 * 端对端测试浮窗
 *
 * 最小化端对端测试弹窗后显示的浮动进度组件。
 * 订阅 E2ETestDialog 的全局状态，实时显示测试进度。
 * 支持拖拽定位和 hover 展开详情。
 *
 * 复制自 BatchTestFloatingWidget，订阅 E2E 全局状态。
 */
import { useState, useEffect, useRef, useCallback } from "react";
import { Button } from "@/components/ui/button";
import { Maximize2, Loader2, GripVertical } from "lucide-react";
import { getE2ETestState, subscribeE2ETestState, type E2ETestState } from "./E2ETestDialog";

interface E2ETestFloatingWidgetProps {
  onExpand: () => void;
  isDialogOpen: boolean;
}

export function E2ETestFloatingWidget({ onExpand, isDialogOpen }: E2ETestFloatingWidgetProps) {
  const [state, setState] = useState<E2ETestState>(() => getE2ETestState());
  ...
  useEffect(() => {
    const unsubscribe = subscribeE2ETestState(() => {
      setState(getE2ETestState());
    });
    return unsubscribe;
  }, []);
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">端对端测试中</span>
        </div>
        <Button
          size="sm"
          variant="ghost"
